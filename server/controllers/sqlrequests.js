    ...
    await sql.connect(sqlConfig, function (err) {
      ...
      requestSalesData.query(querySalesData, function (err, recordset) {
        ...
        salesDataAux = recordset.recordset;
         ...
         if (filter!== 'null') {
          salesDataAux =
          salesDataAux.filter(item => 
            item['Name'].includes(filter) || item['Tenant Name'].includes(filter) )
        } 

        var salesData = Object.values(
          salesDataAux.reduce((r, o) => {
            r[o[dateMode]] = r[o[dateMode]] || {
        ...
        const AssetNames = [
          ...new Set(salesDataAux.map((item) => item["Name"])),
        ];
        const AssetName = AssetNames.length > 1 ? "All" : AssetNames[0];

        const TenantNames = [
          ...new Set(salesDataAux.map((item) => item["Tenant Name"])),
        ];
        ...
        const totalSalesObj = salesData && salesData[salesData.length - 1];
        const totalPreviousSalesObj =
          salesData && salesData.length > 1
            ? salesData[salesData.length - 2]
            : 0;
        //console.log(salesData,totalSalesObj,totalPreviousSalesObj)

        // Calculate the percentage change
        const percentageChange =
          salesData.length > 1
            ? totalPreviousSalesObj.value == 0
              ? totalSalesObj.value * 100
              : ((totalSalesObj.value - totalPreviousSalesObj.value) /
                  totalPreviousSalesObj.value) *
                100
            : 0;
        const salesInfo = {
          AssetName,
          TenantName,
          AssetsAndTenantNames,
          salesData,
          totalSales: parseInt(totalSalesObj.value),
